<template>
  <AppPageLayout>
    <view class="mx-auto flex max-w-[1120px] gap-0 px-6 py-10 lg:px-8 lg:py-14">
      ...
      <!-- 右侧通知详情 -->
      <view class="flex min-h-[500px] flex-1 flex-col border border-l-0 border-semantic-border-default bg-semantic-surface-card shadow-panel">
        <template v-if="activeThread">
          ...
          <view class="flex-1 space-y-4 overflow-y-auto px-6 py-5">
            ...
            >
              <view class="max-w-[70%] rounded-lg bg-semantic-surface-panel px-4 py-3 text-[14px] leading-6 text-semantic-text-primary">
                {{ msg.body }}
              </view>
            </view>
            <view
...
import type { InboxThreadDTO } from '@/api/messages'

const { t } = usePageI18n('messages')
const { threads, messages, activeThreadId, hasMore, selectThread, loadMore } = useMessagesInbox()
...
function resolveLabel(item: InboxThreadDTO) {
  if (item.subjectType) return t(`subject.${item.subjectType}`)
  return t('subject.system')
}

function resolveAvatar(_item: InboxThreadDTO) {
