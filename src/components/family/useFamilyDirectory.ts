...
function matchFamilyMode(profile: MockProfile, value: string) {
  if (!value) return true
...
function getFamilyPriorityRank(profile: MockProfile) {
...
export function useFamilyDirectory(): UseFamilyDirectoryResult<MockProfile> {
  ...
  const cityOptions = computed<DirectoryOption[]>(() => [
    buildBaseAllOption(text.value.all),
    ...getLocalizedProfileOptions(locale.value, profile => profile.city)
      .filter(value => sourceItems.value.some(profile => profile.city[locale.value] === value))
      .map(value => ({
        label: value,
        value,
      })),
  ...
  const intentOptions = computed<DirectoryOption[]>(() => [
    buildBaseAllOption(text.value.all),
    ...getLocalizedIntentOptions(locale.value)
      .filter(item => sourceItems.value.some(profile => profile.intentCode === item.code))
      .map(item => ({
        label: item.label,
        value: item.code,
      })),
  ...
  const occupationOptions = computed<DirectoryOption[]>(() => [
    buildBaseAllOption(text.value.all),
    ...getLocalizedProfileOptions(locale.value, profile => profile.occupation)
      .filter(value => sourceItems.value.some(profile => profile.occupation[locale.value] === value))
      .map(value => ({
        label: value,
        value,
      })),
  ...
  const industryOptions = computed<DirectoryOption[]>(() => [
    buildBaseAllOption(text.value.all),
    ...getLocalizedProfileOptions(locale.value, profile => profile.industry)
      .filter(value => sourceItems.value.some(profile => profile.industry[locale.value] === value))
      .map(value => ({
        label: value,
        value,
      })),
