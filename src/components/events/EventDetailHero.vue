<template>
  <view class="relative overflow-hidden bg-events-hero text-text-inverse">
    <view class="absolute left-[-48px] top-[-28px] h-[220px] w-[220px] rounded-full bg-brand-highlight/12 blur-[84px]" />
    <view class="pointer-events-none absolute right-[-260px] top-1/2 hidden h-[520px] w-[520px] -translate-y-1/2 lg:block">
      <view class="absolute inset-0 rounded-full border border-brand-highlight/16" />
      <view class="absolute inset-[40px] rounded-full border border-brand-highlight-soft/14" />
      <view class="absolute inset-[122px] rounded-full border border-brand-highlight/10" />
      <view class="absolute left-1/2 top-1/2 h-[200px] w-[200px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-brand-highlight/10 blur-[36px]" />
    </view>
    ...
    <view class="relative mx-auto max-w-[1280px] px-8 pb-20 pt-10 lg:pb-24 lg:pt-12">
      <view class="grid gap-10 lg:grid-cols-[0.94fr_1.06fr] lg:items-start">
        <view class="max-w-[700px]">
          <view class="mb-8 inline-flex items-center gap-5 rounded-full border border-border-inverse/34 bg-surface-inverse-panel/34 px-7 py-3 backdrop-blur">
            <view class="h-px w-9 bg-brand-highlight-strong" />
        ...
        <view class="relative overflow-hidden border border-border-inverse/34 bg-surface-inverse-panel/76 px-8 py-8 shadow-hero backdrop-blur">
          <view class="pointer-events-none absolute right-[-54px] top-[-70px] h-[170px] w-[170px] rounded-full bg-brand-highlight/8 blur-[38px]" />

          <view class="flex flex-wrap items-start justify-between gap-4">
            <view class="text-[15px] uppercase tracking-[3px] text-brand-highlight">
              {{ fields.status }}
            </view>
          ...
          <view class="mt-9 grid gap-5 lg:grid-cols-2 lg:gap-x-8">
            ...
            >
              <view class="text-[15px] uppercase tracking-[2.5px] text-brand-highlight-soft/92">
                {{ item.label }}
              </view>
...
const buttonClassName = computed(() => {
  if (props.event.status === 'waitlist') {
    return 'border border-brand-highlight/56 bg-surface-inverse-card/38 text-text-inverse hover:-translate-y-[1px] hover:border-brand-highlight/72 hover:bg-surface-inverse-card/54 hover:shadow-card'
  }
  ...
  return 'border border-button-highlight bg-button-highlight text-button-neutral-ink hover:-translate-y-[1px] hover:border-button-highlight-hover hover:bg-button-highlight-hover hover:shadow-card'
})
